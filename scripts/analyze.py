...
def plot_predicted_vs_actual(model, dataloader, output_dir, num_batches=50):
    """Plot predicted vs actual returns."""
    print("Plotting predicted vs actual returns...")
    output_dir = Path(output_dir)
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            # Predicted mean = alpha + B^T @ mu_z
            pred_mean = alpha + torch.matmul(B, mu_z.unsqueeze(-1)).squeeze(-1)
            
            all_pred.append(pred_mean.cpu().numpy()[0])
            all_actual.append(r.cpu().numpy()[0])
    ...
    plt.scatter(all_actual, all_pred, alpha=0.3, s=1)
    ...
    stats.probplot(all_actual - all_pred, dist="norm", plot=ax)
    ax.set_title('Q-Q Plot of Prediction Errors')
    plt.tight_layout()
    plt.savefig(output_dir / 'qq_plot.png', dpi=300)
    plt.close()
    
    # Calculate metrics
    mse = np.mean((all_pred - all_actual) ** 2)
...
def main():
    ...
    plot_predicted_vs_actual(model, dataloader, output_dir)
